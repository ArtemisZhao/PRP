...
out_name = paste0("sim_data/batch_2grp_batch_sd_",round(bb_sd,2),"_noise_sd_",rnse,".dat")
write(file= out_name, ncol=5, t(outd))

pvec = sapply(1:5000, function(x) prior_prp(beta=c(rst[x,1], rst[x,3]), se=c(rst[x,2], rst[x,4]), test="two_sided")$pvalue)

outd = cbind(1:5000, pvec)
write(file=paste0("output/batch_2gp_bb_sd_1_noise_sd_",rnse, ".prp.out"), t(outd), ncol=2)
