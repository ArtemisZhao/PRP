...
#k_vec=c(0.272612228503996)

k_max = max(k_vec)

res=list()
prior_PRP_mcmc <- function (beta, se , nreps = 50000, burnin_rate = 0.01){
  ...
  betar = sapply(1:length(bbar_sample), function(x) rnorm(1, mean=bbar_sample[x], sd = sqrt(bbar_sample[x]^2*k_sample[x]^2+sr^2)))
  ...
  #}
  #else{
  #  p1= 1- sapply(1:length(bbar_sample), function(x) pnorm(hbr, mean=bbar_sample[x], sd = sqrt(bbar_sample[x]^2*k_sample[x]^2+sr^2)))
  #}
  p2 = 1 - p1
  
  pval1 = 2*min(apply(cbind(p1,p2),2, mean))
  #pval1=mean(p1)
  pval2 = 2* min(length(which(betar>hbr))/length(betar),1-length(which(betar>hbr))/length(betar))
  
  left_bound= quantile(betar,0.025)
  right_bound = quantile(betar,0.975)
  #cat("p-value = ", pval,"\n")
  res[["pval"]] = pval1
  res[["bbar_sample"]] = bbar_sample
  res[["betar_sample"]] = betar
  res[["pval_sample"]] = pval2
  res[["left_bound"]] =left_bound
  res[["right_bound"]] =right_bound
  return(res)
}
